...
### Added — Weather
- **Forecast rows show each day's moon phase**: A small phase glyph sits next to the weather icon on every multi-day forecast row. Phase is computed locally via `suncalc.getMoonIllumination` (phase angle is global, no lat/lon needed), sampled at the day's local noon to dodge edge-of-day rollover. Same shape vocabulary as the on-chart glyph: filled circle for full, empty outlined circle for new, crescent / gibbous for the in-betweens.
- **Daylight duration tinted amber, moon phase name now sits under its arc apex**: The "Xh Ym" label between sunrise and sunset is now amber to match the sun arc instead of muted gray. The moon phase name moves from chart-center to the midpoint between moonrise and moonset (the moon's apex) when both are in today's window, falling back to chart-center only for polar edge cases where rise or set straddle midnight.
- **Sun + moon share one altitude arc**: The daylight chart in the weather widget now plots both bodies on the same 24-hour timeline, with peak heights driven by true celestial altitudes from `suncalc` (zenith = full arc height, sub-zenith proportionally smaller) — so summer sun visibly arcs higher than winter sun, and the moon arc tracks its declination. Sun arc colored by altitude via an SVG `linearGradient` — red at the horizon, orange at low altitude (~25°), amber at zenith — matching the atmospheric-scattering color shift you'd actually see in the sky. Elapsed/future and above/below distinctions preserved. Sun dot's fill follows the same altitude bucketing so a low sun glows red/orange. Moon is blue when above horizon, muted slate below. Moon glyph at current position renders the actual phase shape: full = filled circle, new = outlined empty circle, crescents and gibbous show the lit fraction. `WeatherData` now carries `moonrise`, `moonset`, `moonPhase`, `moonIllumination`, `moonPhaseName`, `lat`, `lon` from all three providers (Open-Meteo, OpenWeatherMap, Pirate Weather) via a shared `src/lib/integrations/moon.ts` helper — `suncalc` is purely local, no API key or network call.
