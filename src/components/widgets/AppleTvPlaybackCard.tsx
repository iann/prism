...
export function AppleTvPlaybackCard({
  ...
  const estimatedPlayback = estimateHomeAssistantAppleTvPosition(data, now);
  ...
  if (!enabled || !data.visible || !data.active) return null;
  ...
  const deviceName = data.deviceName || 'Apple TV';
  return (
    <>
      <Card
        className={cn(
