...
export type SupportedAppleTvControl =
  | 'play'
  | 'pause'
  | 'play_pause'
  | 'previous'
...
export function normalizeHomeAssistantAppleTvState(raw: RawState): HomeAssistantAppleTvState {
  ...
  const supportedControls: SupportedAppleTvControl[] = [];
...
export function buildHomeAssistantAppleTvActionRequests(
  input: HomeAssistantAppleTvActionInput,
  mediaPlayerEntityId: string
): HomeAssistantServiceRequest[] {
  const id = validEntity(mediaPlayerEntityId, 'media_player');
  if (typeof input !== 'object' || input === null) throw new Error('Invalid Apple TV action');
  if (
    input.control === 'seek' &&
  ...
  switch (input.control) {
    ...
    // Requests are ordered deliberately; callers can report partial failure by requestId.
    case 'turn_off': {
      // Sleep is deliberately a two-step operation: suspend the remote, then turn off the player.
      ...
      return [
        ...
        media('turn_off', id, {}, 'turn_off'),
      ];
    }
  }
  throw new Error('Invalid Apple TV action control');
}

/** End time is estimated from `now` and the resulting position, and is null without a valid duration. */
export function estimateHomeAssistantAppleTvPosition(
  state: Pick<HomeAssistantAppleTvState, 'position' | 'positionUpdatedAt' | 'duration' | 'state'>,
