...
describe('projectCommunityLayout', () => {
  ...
  it('preserves canonical widget types for v3 multiple-instance exports', () => {
    const out = projectCommunityLayout({
      ...valid,
      version: 3,
      widgets: [{ i: 'weather-2', type: 'weather', x: 0, y: 0, w: 8, h: 4 }],
    });

    expect(out.version).toBe(3);
    expect(out.widgets[0]).toMatchObject({ i: 'weather-2', type: 'weather' });
  });
  ...
  it('carries a valid version and normalises anything else to the current one', () => {
    // Preserve v1-v3 for compatibility, and normalize unknown versions to v3
    // so a personal export is never silently downgraded.
    expect(projectCommunityLayout({ ...valid, version: 2 }).version).toBe(2);
    expect(projectCommunityLayout({ ...valid, version: 1 }).version).toBe(1);
    expect(projectCommunityLayout({ ...valid, version: 3 }).version).toBe(3);
    expect(projectCommunityLayout({ ...valid, version: 99 }).version).toBe(3);
    expect(projectCommunityLayout({ ...valid, version: undefined }).version).toBe(3);
  });
