...
function normalizeWidgets(widgets: CommunityWidget[]): string {
  const sorted = [...widgets]
    .sort((a, b) => {
      const typeA = a.type || a.i;
      const typeB = b.type || b.i;
      return typeA.localeCompare(typeB) || a.x - b.x || a.y - b.y;
    })
    .map((w) => ({
      type: w.type || w.i,
      x: Math.round(w.x),
...
export function projectCommunityLayout(
  ...
  const widgets: CommunityWidget[] = (Array.isArray(obj.widgets) ? obj.widgets : []).map((raw) => {
    ...
    // v3 exports use canonical `type` values with instance-specific IDs (for
    // example `weather-2`). Preserve that distinction so CI can validate and
    // publish multiple instances without treating the ID as the widget type.
    if (typeof w.type === 'string') widget.type = w.type;
  ...
  return {
    type: 'prism-layout',
    // Carried, not hardcoded. Keep the personal v3 export path intact while
    // accepting the older community formats; an unrecognised version becomes
    // the current one instead of riding through.
    version: obj.version === 1 || obj.version === 2 || obj.version === 3 ? obj.version : 3,
...
export function validateCommunityLayout(
  ...
  if (typeof obj.version !== 'number' || ![1, 2, 3].includes(obj.version)) {
    errors.push('Missing or invalid "version" field. Expected 1, 2, or 3.');
  }
  ...
  for (let idx = 0; idx < widgets.length; idx++) {
    ...
    const widgetType = typeof wObj.type === 'string' ? wObj.type : (wObj.i as string);
    const widget: CommunityWidget = {
      i: wObj.i as string,
      type: widgetType,
      x: wObj.x as number,
    ...
    // Valid widget ID
    if (!VALID_WIDGET_IDS.includes(widgetType)) {
      errors.push(
        `Widget "${widgetType}": unknown widget type. Valid types: ${VALID_WIDGET_IDS.join(', ')}.`
      );
    }
