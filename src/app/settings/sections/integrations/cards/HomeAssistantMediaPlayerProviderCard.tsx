...
export function HomeAssistantMediaPlayerProviderCard() {
  ...
  const validate = () => {
    ...
    if (!/^media_player\.[a-z0-9_-]+$/.test(mediaPlayer.trim()))
      return 'Media player must look like media_player.living_room.';
    if (remote.trim() && !/^remote\.[a-z0-9_-]+$/.test(remote.trim()))
      return 'Remote must look like remote.living_room when provided.';
    return null;
  };
  ...
  const payload = () => ({
    ...
    remoteEntityId: remote.trim() || null,
  });
  ...
  const request = async (action: 'discover' | 'test' | 'save') => {
    ...
    try {
      ...
      if (action === 'discover') {
        ...
        setMessage('Discovery complete. Choose a media player and optional remote below.');
      } else {
        setMessage(
  ...
  return (
    <ProviderCardShell
      id="home-assistant-media-player"
      ...
      description="Control a media player through your Home Assistant instance."
      primaryAction={
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="Optional: remote.living_room"
            />
            <p className="mt-1 text-xs text-muted-foreground">
              Optional; enables Apple TV-style suspend before power off.
            </p>
          </div>
        </div>
        {candidates.length > 0 && (
