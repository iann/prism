  ...
  test('band titles and card titles start at the same offset', async ({ page }) => {
    ...
    const offsets = await page.evaluate(() => {
      ...
      const out: Array<{
        ...
        slice: ReturnType<typeof describe>;
        cardEl?: ReturnType<typeof describe>;
      }> = [];
      document.querySelectorAll('[data-spanning-events]').forEach((band) => {
        ...
        // Timed one-day events use the shared lane too, so when there is no
        // separate event card below the lane, compare the title to its own
        // stripe and flex gap instead.
        const card = [...cell.querySelectorAll('button[data-variant="event"]')].find(
    ...
    const misaligned = offsets.filter(
      (o) => Math.abs(o.band - (o.card ?? o.expectedBand!)) > 0.5);
